...
function drawSphere(canvas, data, strike, dip, rake, options = {}) {
  ...
  for (let idx = 0; idx < data.length; idx += 1) {
    ...
    if (options.highlightIndex === idx) {
      ctx.save();
      ctx.beginPath();
      ctx.arc(pos.x, pos.y, 18, 0, Math.PI * 2);
      ctx.fillStyle = 'rgba(198, 151, 73, 0.22)';
      ctx.strokeStyle = '#c69749';
      ctx.lineWidth = 3.2;
      ctx.fill();
      ctx.stroke();
      ctx.restore();
...
function playStationScan() {
  if (!picks.length) {
    $('stationNarration').textContent = '請先載入或整理 P25 資料。';
    return;
  }
  stopStationScan(false);
  highlightIndex = -1;
  stationTimer = setInterval(() => {
    highlightIndex = (highlightIndex + 1) % picks.length;
    const p = picks[highlightIndex];
    const fix = p.flipped
      ? `take-off ${p.takeoff.toFixed(1)}° > 90°，所以修正成 ${p.ctakeoff.toFixed(1)}°，azimuth 修正成 ${p.cazimuth.toFixed(1)}°。`
      : `take-off ${p.takeoff.toFixed(1)}° 未超過 90°，可以直接使用原方位。`;
    $('stationNarration').textContent = `${p.station}：azimuth=${p.azimuth.toFixed(1)}°，take-off=${p.takeoff.toFixed(1)}°，初動=${p.polarity}。${fix}`;
    refresh();
  }, 760);
}

function stopStationScan(reset = true) {
  if (stationTimer) clearInterval(stationTimer);
  stationTimer = null;
  if (reset) {
    highlightIndex = -1;
    $('stationNarration').textContent = '播放已停止。可再次按「逐點播放測站」觀察點位。';
    refresh();
  }
}

function drawCorrectionLab() {
  const canvas = $('correctionCanvas');
  if (!canvas) return;
  const ctx = canvas.getContext('2d');
  const W = canvas.width, H = canvas.height;
  const cx = W * 0.62, cy = H * 0.52, radius = Math.min(W, H) * 0.36;
  const az = Number($('corrAz').value);
  const take = Number($('corrTake').value);
  const pol = $('corrPolarity').value;
  const corr = correctTakeoff(az, take);
  $('corrAzOut').textContent = `${az}°`;
  $('corrTakeOut').textContent = `${take}°`;
  $('corrFormula').textContent = corr.flipped
    ? `take-off 超過 90°：${take}° → 180°-${take}°=${corr.takeoff.toFixed(1)}°；azimuth：${az}°-180°=${corr.azimuth.toFixed(1)}°。`
    : `take-off 未超過 90°：直接使用 azimuth=${az.toFixed(1)}°、take-off=${take.toFixed(1)}°。`;

  ctx.clearRect(0, 0, W, H);
  ctx.fillStyle = '#fffdf8';
  ctx.fillRect(0, 0, W, H);
  ctx.save();
  ctx.strokeStyle = '#dfd0be'; ctx.lineWidth = 1.4;
  ctx.beginPath(); ctx.arc(cx, cy, radius, 0, Math.PI * 2); ctx.stroke();
  ctx.beginPath(); ctx.moveTo(cx, cy - radius); ctx.lineTo(cx, cy + radius); ctx.stroke();
  ctx.beginPath(); ctx.moveTo(cx - radius, cy); ctx.lineTo(cx + radius, cy); ctx.stroke();
  ctx.fillStyle = '#241912'; ctx.font = 'bold 18px system-ui'; ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
  ctx.fillText('N', cx, cy - radius - 22); ctx.fillText('E', cx + radius + 22, cy); ctx.fillText('S', cx, cy + radius + 22); ctx.fillText('W', cx - radius - 22, cy);

  const raw = projectAzTake(az, take, radius, cx, cy);
  const fixed = projectAzTake(corr.azimuth, corr.takeoff, radius, cx, cy);
  ctx.setLineDash([7, 7]);
  ctx.strokeStyle = corr.flipped ? '#8f3d2f' : '#c69749';
  ctx.beginPath(); ctx.moveTo(raw.x, raw.y); ctx.lineTo(fixed.x, fixed.y); ctx.stroke();
  ctx.setLineDash([]);

  ctx.fillStyle = '#8f3d2f'; ctx.font = '800 15px system-ui'; ctx.textAlign = 'left';
  ctx.fillText('原始輸入位置', 26, 54);
  ctx.fillStyle = '#74665b'; ctx.font = '14px system-ui';
  ctx.fillText(`az=${az.toFixed(0)}° / take-off=${take.toFixed(0)}°`, 26, 78);
  ctx.fillStyle = '#c69749'; ctx.font = '800 15px system-ui';
  ctx.fillText('下半球正確位置', 26, 132);
  ctx.fillStyle = '#74665b'; ctx.font = '14px system-ui';
  ctx.fillText(`az=${corr.azimuth.toFixed(1)}° / take-off=${corr.takeoff.toFixed(1)}°`, 26, 156);

  ctx.beginPath(); ctx.arc(raw.x, raw.y, 8, 0, Math.PI * 2);
  ctx.fillStyle = corr.flipped ? 'rgba(143, 61, 47, .25)' : 'rgba(198, 151, 73, .45)';
  ctx.strokeStyle = '#8f3d2f'; ctx.lineWidth = 2; ctx.fill(); ctx.stroke();

  ctx.beginPath(); ctx.arc(fixed.x, fixed.y, pol === '+' ? 8 : 10, 0, Math.PI * 2);
  ctx.fillStyle = pol === '+' ? '#111' : '#fff';
  ctx.strokeStyle = '#111'; ctx.lineWidth = pol === '+' ? 1.2 : 2.4; ctx.fill(); ctx.stroke();
  ctx.fillStyle = '#241912'; ctx.font = 'bold 14px system-ui'; ctx.textAlign = 'center';
  ctx.fillText(pol === '+' ? '+ 實心' : '− 空心', fixed.x, fixed.y - 18);

  if (corr.flipped) {
    ctx.fillStyle = '#8f3d2f'; ctx.font = 'bold 14px system-ui'; ctx.textAlign = 'center';
    ctx.fillText('超過 90°，需翻到對側方位', (raw.x + fixed.x) / 2, (raw.y + fixed.y) / 2 - 12);
  }
  ctx.restore();
}
...
const PRINCIPLE_STEPS = [
  {
    title: 'Step 1：每個測站是一條從震源射出的射線',
    text: 'P 波從震源向外傳到測站。azimuth 決定射線在水平面的方向，take-off angle 決定射線離垂直方向有多遠。'
  },
  {
    title: 'Step 2：把射線投影到下半球平面',
    text: '下半球等面積投影把三維方向壓到一張圓圖上。take-off angle 越大，點越接近外圈；越小，點越接近中心。'
  },
  {
    title: 'Step 3：+ / − 初動形成壓縮與擴張分區',
    text: '+ 可視為壓縮初動，常畫成實心點；− 可視為擴張初動，常畫成空心點。多個測站會逐漸形成黑白分布。'
  },
  {
    title: 'Step 4：找出能分開黑白點的兩條節面',
    text: '雙偶震源的 P 波輻射型態會有兩條振幅接近零的節面。手繪靠視覺分區，程式則計算 strike、dip、rake 哪一組能讓初動符合率最高。'
  }
];

function drawMiniSphere(ctx, cx, cy, radius, strike, dip, rake, beach = false, npts = 0) {
  const M = momentComponents(strike, dip, rake);
  if (beach) {
    const img = ctx.createImageData(Math.ceil(radius * 2.2), Math.ceil(radius * 2.2));
    const w = img.width, h = img.height, pcx = w / 2, pcy = h / 2;
    for (let py = 0; py < h; py++) {
      for (let px = 0; px < w; px++) {
        const x = (px - pcx) / radius;
        const y = (pcy - py) / radius;
        const idx = (py * w + px) * 4;
        if (x*x + y*y <= 1) {
          const a = amplitude(inverseEqualArea(x, y), M);
          const c = a >= 0 ? 224 : 255;
          ...
          img.data[idx] = 255; img.data[idx+1] = 253; img.data[idx+2] = 248; img.data[idx+3] = 0;
        }
      }
    }
    ctx.putImageData(img, cx - pcx, cy - pcy);
  }
  ctx.beginPath(); ctx.arc(cx, cy, radius, 0, Math.PI * 2); ctx.lineWidth = 3; ctx.strokeStyle = '#111'; ctx.stroke();
  ctx.font = 'bold 16px system-ui'; ctx.textAlign = 'center'; ctx.textBaseline = 'middle'; ctx.fillStyle = '#111';
  ctx.fillText('N', cx, cy-radius-20); ctx.fillText('E', cx+radius+20, cy); ctx.fillText('S', cx, cy+radius+20); ctx.fillText('W', cx-radius-20, cy);
  const rng = mulberry32(9090);
  for (let i=0; i<npts; i++) {
    const az = norm360(i * 360 / npts + randRange(rng, -7, 7));
    const take = randRange(rng, 18, 84);
    const v = vectorFromAzTake(az, take);
    const pol = amplitude(v, M) >= 0 ? '+' : '-';
    const pos = projectAzTake(az, take, radius, cx, cy);
    ctx.beginPath(); ctx.arc(pos.x, pos.y, pol === '+' ? 5.5 : 7, 0, Math.PI*2);
    ctx.fillStyle = pol === '+' ? '#111' : '#fff'; ctx.strokeStyle = '#111'; ctx.lineWidth = pol === '+' ? 1 : 2; ctx.fill(); ctx.stroke();
  }
}

function drawPrinciple() {
  const canvas = $('principleCanvas');
  if (!canvas) return;
  const ctx = canvas.getContext('2d');
  const W = canvas.width, H = canvas.height;
  ctx.clearRect(0,0,W,H);
  ...
  const source = {x: 130, y: H * 0.55};
  const station = {x: 330, y: 105};
  const cx = 560, cy = H * 0.53, radius = 145;
  const step = PRINCIPLE_STEPS[principleStep];
  $('principleTitle').textContent = step.title;
  $('principleText').textContent = step.text;

  ctx.fillStyle = '#f6ead9'; ctx.strokeStyle = '#dfd0be'; ctx.lineWidth = 2;
  ctx.beginPath(); ctx.arc(source.x, source.y, 48, 0, Math.PI*2); ctx.fill(); ctx.stroke();
  ctx.fillStyle = '#8f3d2f'; ctx.beginPath(); ctx.arc(source.x, source.y, 8, 0, Math.PI*2); ctx.fill();
  ctx.fillStyle = '#241912'; ctx.font = 'bold 15px system-ui'; ctx.textAlign = 'center'; ctx.fillText('震源', source.x, source.y+70);

  ctx.strokeStyle = '#8f3d2f'; ctx.lineWidth = 3; ctx.setLineDash([9,6]);
  ctx.beginPath(); ctx.moveTo(source.x, source.y); ctx.lineTo(station.x, station.y); ctx.stroke(); ctx.setLineDash([]);
  const px = source.x + (station.x-source.x)*t;
  const py = source.y + (station.y-source.y)*t;
  ctx.fillStyle = '#c69749'; ctx.beginPath(); ctx.arc(px, py, 7, 0, Math.PI*2); ctx.fill();
  ctx.fillStyle = '#241912'; ctx.font = 'bold 15px system-ui'; ctx.fillText('測站', station.x, station.y-18);
  ctx.beginPath(); ctx.rect(station.x-13, station.y-13, 26, 26); ctx.strokeStyle = '#241912'; ctx.lineWidth = 2; ctx.stroke();

  if (principleStep >= 1) {
    ctx.strokeStyle = '#c69749'; ctx.lineWidth = 2;
    ctx.beginPath(); ctx.moveTo(380, H*0.54); ctx.lineTo(cx-radius-28, cy); ctx.stroke();
    drawMiniSphere(ctx, cx, cy, radius, 102, 30, -11, principleStep >= 3, principleStep >= 2 ? 26 : 0);
    const pos = projectAzTake(125, 55, radius, cx, cy);
    ctx.beginPath(); ctx.arc(pos.x, pos.y, 8, 0, Math.PI*2); ctx.fillStyle = '#111'; ctx.fill(); ctx.strokeStyle = '#111'; ctx.stroke();
    ctx.fillStyle = '#8f3d2f'; ctx.font = 'bold 14px system-ui'; ctx.textAlign = 'left'; ctx.fillText('單站投影點', pos.x+12, pos.y-8);
  }

  if (principleStep >= 3) {
    ctx.fillStyle = 'rgba(143,61,47,.08)'; ctx.fillRect(22, 25, 334, 78);
    ctx.fillStyle = '#241912'; ctx.font = 'bold 15px system-ui'; ctx.textAlign = 'left';
    ctx.fillText('重點：節面不是直接量一條線，而是找出 P 波振幅由 + 變 − 的邊界。', 38, 58);
    ctx.fillText('手繪是目測分界；程式是逐組 strike / dip / rake 計算符合率。', 38, 84);
  }
}

function startPrincipleAnimation() {
  if (principleAnimationId) cancelAnimationFrame(principleAnimationId);
  const loop = () => { drawPrinciple(); principleAnimationId = requestAnimationFrame(loop); };
  loop();
}
...
function makeGame() {
  ...
  gameSketch = [];
  gameHintLevel = 0;
  if ($('gameHintBox')) $('gameHintBox').textContent = '逆衝型常見中心壓縮；正斷層常見中心擴張；走向滑移通常接近四象限棋盤狀；斜移型則會歪斜、不完全對稱。';
  drawGame();
}
...
function drawGameSketch() {
  ...
  const pairs = [[0,1],[2,3]];
  ctx.save();
  ctx.strokeStyle = '#8f3d2f';
  ctx.fillStyle = '#8f3d2f';
  ctx.lineWidth = 3.2;
  for (const [a,b] of pairs) {
    if (gameSketch[a]) {
      ctx.beginPath(); ctx.arc(gameSketch[a].x, gameSketch[a].y, 6, 0, Math.PI*2); ctx.fill();
    }
    if (gameSketch[a] && gameSketch[b]) {
      ctx.beginPath(); ctx.moveTo(gameSketch[a].x, gameSketch[a].y); ctx.lineTo(gameSketch[b].x, gameSketch[b].y); ctx.stroke();
      ctx.font = 'bold 16px system-ui'; ctx.fillText(a === 0 ? '你的節面 A' : '你的節面 B', (gameSketch[a].x + gameSketch[b].x) / 2 + 8, (gameSketch[a].y + gameSketch[b].y) / 2 - 8);
    }
  }
  ctx.restore();
}

function addSketchPoint(evt) {
  const canvas = $('gameCanvas');
  const rect = canvas.getBoundingClientRect();
  const x = (evt.clientX - rect.left) * canvas.width / rect.width;
  const y = (evt.clientY - rect.top) * canvas.height / rect.height;
  const cx = canvas.width / 2, cy = canvas.height / 2, radius = Math.min(canvas.width, canvas.height) * 0.41;
  if ((x-cx)*(x-cx) + (y-cy)*(y-cy) > radius*radius) return;
  if (gameSketch.length >= 4) gameSketch = [];
  gameSketch.push({x, y});
  $('gameFeedback').textContent = gameSketch.length < 4
    ? `已放置第 ${gameSketch.length} 個節面控制點；共需 4 個點可畫兩條猜測節面。`
    : '已畫出兩條猜測節面。現在可以選擇斷層型態並檢查答案。';
  drawGame();
}

function giveGameHint() {
  if (!currentGame) makeGame();
  gameHintLevel = (gameHintLevel % 3) + 1;
  const answer = classifyFault(currentGame.dip, currentGame.rake);
  const centerAmp = amplitude(vectorFromAzTake(0, 0), momentComponents(currentGame.strike, currentGame.dip, currentGame.rake));
  const center = centerAmp >= 0 ? '中心附近偏壓縮（黑色）' : '中心附近偏擴張（白色）';
  const hints = [
    `提示 1：先看中心區，這題${center}；再觀察外圈是否形成四象限。`,
    `提示 2：這題的節面分界${answer.includes('走向滑移') ? '接近四象限棋盤狀' : answer.includes('斜移') ? '呈現歪斜的斜移型分布' : '較接近傾向滑移型的黑白分布'}。`,
    `提示 3：答案很接近「${answer}」。試著打開正解海灘球確認節面位置。`
  ];
  $('gameHintBox').textContent = hints[gameHintLevel - 1];
}
...
function bindEvents() {
  ...
  $('playStations').addEventListener('click', playStationScan);
  $('stopStations').addEventListener('click', () => stopStationScan(true));
  ['strike','dip','rake','showBeach','showLabels','showMisfit'].forEach(id => $(id).addEventListener('input', refresh));
  ['corrAz','corrTake','corrPolarity'].forEach(id => $(id).addEventListener('input', drawCorrectionLab));
  ...
  $('hintGame').addEventListener('click', giveGameHint);
  $('clearSketch').addEventListener('click', () => { gameSketch = []; $('gameFeedback').textContent = '已清除你畫的節面，可重新在圖上點 4 個點。'; drawGame(); });
  $('gameCanvas').addEventListener('click', addSketchPoint);
  $('prevPrinciple').addEventListener('click', () => { principleStep = (principleStep + PRINCIPLE_STEPS.length - 1) % PRINCIPLE_STEPS.length; drawPrinciple(); });
  $('nextPrinciple').addEventListener('click', () => { principleStep = (principleStep + 1) % PRINCIPLE_STEPS.length; drawPrinciple(); });
}
